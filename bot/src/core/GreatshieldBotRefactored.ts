...
export class GreatshieldBot {
  ...
  private async onReady(): Promise<void> {
    ...
    for (const [guildId] of this.client.guilds.cache) {
      try {
        const config = await this.db.getBotConfig(guildId);
        if (config && config.is_enabled !== false) {
          this.config = config;
          console.log(`📋 Loaded configuration for guild: ${this.client.guilds.cache.get(guildId)?.name || guildId}`);

          // Initialize moderation pipeline
          await this.moderationPipeline.initialize(config);
          break; // For now, only handle the first configured guild
        }
      } catch (error) {
        this.logger.error('Failed to load configuration for guild', {
          guildId,
          error: String(error)
        });
      }
