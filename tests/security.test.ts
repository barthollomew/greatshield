...
import { InputValidator } from '../bot/src/security/InputValidator';
import { RateLimiter, RateLimitConfig } from '../bot/src/security/RateLimiter';
import { ContentSanitizer, SanitizationConfig } from '../bot/src/security/ContentSanitizer';
import { DatabaseManager } from '../bot/src/database/DatabaseManager';
...
describe('Security System Tests', () => {
  ...
  describe('InputValidator', () => {
    ...
    it('should detect SQL injection attempts', async () => {
      ...
      expect(result.errors.some((e: string) => e.includes('injection'))).toBe(true);
    });
    ...
    it('should detect zero-width characters', async () => {
      ...
      expect(result.errors.some((e: string) => e.includes('Zero-width'))).toBe(true);
    });
    ...
    it('should validate attachments', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.riskLevel).toBe('critical');
      expect(result.errors.some((e: string) => e.includes('Dangerous file type'))).toBe(true);
    });
    ...
    it('should detect repeated character spam', async () => {
      ...
      expect(result.riskLevel).toBe('medium');
      expect(result.errors.some((e: string) => e.includes('spam'))).toBe(true);
    });
  ...
  describe('ContentSanitizer', () => {
    ...
    it('should block SQL injection attempts', async () => {
      ...
      expect(result.riskLevel).toBe('critical');
      expect(result.blockedElements.some((e: string) => e.includes('SQL'))).toBe(true);
    });
    ...
    it('should block XSS attempts', async () => {
      ...
      expect(result.riskLevel).toBe('critical');
      expect(result.blockedElements.some((e: string) => e.includes('XSS'))).toBe(true);
    });
    ...
    it('should remove dangerous HTML tags', async () => {
      ...
      expect(result.riskLevel).toBe('high');
      expect(result.modificationsApplied.some((m: string) => m.includes('HTML'))).toBe(true);
    });
    ...
    it('should normalize unicode characters', async () => {
      ...
      expect(result.modificationsApplied.some((m: string) => m.includes('Zero-width'))).toBe(true);
    });
    ...
    it('should sanitize malicious URLs', async () => {
      ...
      expect(result.blockedElements.some((e: string) => e.includes('URL'))).toBe(true);
    });
    ...
    it('should remove excessive whitespace', async () => {
      ...
      expect(result.modificationsApplied.some((m: string) => m.includes('whitespace'))).toBe(true);
    });
    ...
    it('should reduce repeated characters', async () => {
      ...
      expect(result.modificationsApplied.some((m: string) => m.includes('Repeated'))).toBe(true);
    });
    ...
    it('should truncate overly long content', async () => {
      ...
      expect(result.modificationsApplied.some((m: string) => m.includes('truncated'))).toBe(true);
    });
    ...
    it('should handle null bytes', async () => {
      ...
      expect(result.sanitizedContent).toBe('HelloWorld');
      expect(result.modificationsApplied.some((m: string) => m.includes('Null bytes'))).toBe(true);
      expect(result.riskLevel).toBe('high');
    });
